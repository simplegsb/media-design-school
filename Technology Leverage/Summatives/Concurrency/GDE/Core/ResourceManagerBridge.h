/**
 *	@file GDE\Core\ResourceManagerBridge.h
 *	@brief 
 *	
 *	Copyright()
...
namespace GDE
{
	 ...
	 *
	 *	The iDLL_ID should simply be the same one as is used for the manager that is responsible for owning the resource manager.
	 *	@see TResourceBridge.
	 *
	 ...
	 *
	 *	@tparam TManagerType	The subclassing manager type.
	 *	@tparam TImplementer	The TBridge#TImplementer
	 *	@tparam	TResourceBridge	The type of the resource bridge. @see GDE::TResourceBridge.
	 */
	 ...
	 *
	 *	@param ResId		A unique id containing the filename of the resource to load.
			...
			const CResourceId&	ResId )
	{
		...
		if ( NULL != TParent::m_pImplementer )
		{
			...
			if ( reinterpret_cast< TIResourceManagerImp< typename TResourceBridge::TImplementer > * >(TParent::m_pImplementer)->LoadResource( ResId.GetFileName(), pNakedResource ) )
			{
				++m_uResourceCount;
				m_pResourceList = new TResourceBridge( pNakedResource, ResId, reinterpret_cast<TManagerType*>(this), m_pResourceList );
		...
		TResourceBridge* pResource )
	{
		...
		if ( pResource->m_pImplementer )
		{
			reinterpret_cast< TIResourceManagerImp< typename TResourceBridge::TImplementer > * >(TParent::m_pImplementer)->FreeResource( pResource->m_pImplementer );
		}
		delete pResource;
		--m_uResourceCount;
		return true;
