...
//#define _ADDITIONAL_RUN_TIME_COUNTS // TODO: recreate this functionality.
...
// we at MDS don't want these as it requires including windows.h or windef.h - yuck.
#ifdef _WINDEF_
...
#endif // _WINDEF_ included.

#ifdef _ADDITIONAL_RUN_TIME_COUNTS
struct SVLDStats
{
	size_t		 m_uCurrentMemoryUse;
	size_t		 m_uMaxMemoryUse;
	unsigned int m_uCurrentAllocs;
	unsigned int m_uMaxActiveAllocs;
	unsigned int m_uTotalAllocs;
	unsigned int m_uNumReallocs;
	unsigned int m_uNumFrees;
};
__declspec(dllimport) void VLDResetCounts ();
__declspec(dllimport) void VLDOutputCounts ();
__declspec(dllimport) void VLDGetCounts ( SVLDStats* pStats );
#endif
