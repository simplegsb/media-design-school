...
BOOL CALLBACK
DialogWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch(msg)
	{
		case WM_CLOSE:
		...
		case WM_INITDIALOG:
		{
			CheckRadioButton(hwnd, IDC_WINDOWED, IDC_FULLSCREEN, IDC_WINDOWED);
			return true;
		}
		case WM_COMMAND:
		{
			switch(LOWORD(wParam))
			{
				case IDC_ADAPTER:
				{
					app->toggleShowAdapter();
					break;
				}
				case IDC_DATE_TIME:
				{
					app->toggleShowDateTime();
					break;
				}
				case IDC_FULLSCREEN:
				{
...
WindowApp::WindowApp(HINSTANCE hInstance)
{
	...
	mClearColor     = D3DXCOLOR(1.0f, 0.5f, 0.0f, 1.0f);
	mClientWidth    = 800;
	mClientHeight   = 600;
}
...
int WindowApp::run()
{
	...
	while(msg.message != WM_QUIT)
	{
		...
		if(PeekMessage( &msg, 0, 0, 0, PM_REMOVE ))
		{
			...
			if(mhDialogWnd == NULL || !IsDialogMessage(mhDialogWnd, &msg))
			{
...
void WindowApp::initDialogWindow()
{
	mhDialogWnd = CreateDialog(mhAppInst, MAKEINTRESOURCE(IDD_CONTROL_PANEL), NULL, DialogWndProc);
	ShowWindow(mhDialogWnd, SW_SHOW);
}
...
void WindowApp::toggleShowAdapter()
{
	mShowAdapter = !mShowAdapter;
}

void WindowApp::toggleShowDateTime()
{
	mShowDateTime = !mShowDateTime;
}
